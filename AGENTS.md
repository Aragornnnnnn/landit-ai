...
- 현재 public API는 `GET /health`와 `/api/v1/conversation` 아래의 `next-message`, `inner-thought`, `closing-message`, `message-feedback`, `session-feedback`입니다.
...
- PR 생성 시 변경 성격에 맞는 label을 설정합니다.
