...
- Landit의 `next-question`, `turn-feedback`, `session-feedback`, `guide` 같은 LLM 기반 생성 기능을 담당합니다.
...
- prompts, postprocess, scoring, JSON parsing은 섞지 않고 분리합니다.

현재 구현된 public API는 `GET /health`뿐입니다. 위 생성 API들은 앞으로 이 서버가 맡을 책임 방향이며, 아직 구현된 엔드포인트가 아닙니다.
...
- `llm`은 OpenAI SDK와 OpenRouter 호출의 단일 진입점입니다.
- `prompts`, `postprocess`, `scoring`, JSON parsing은 각각 독립된 모듈로 둡니다.
...
아직 `conversation`, `domain`, `llm`, `prompts`, `postprocess`, `scoring` 패키지는 없습니다. 생성 API를 구현할 때 위 아키텍처 방향에 맞춰 필요한 패키지만 추가합니다.
