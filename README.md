...
현재 구현된 public API는 `GET /health`, `POST /api/v1/conversation/next-message`, `POST /api/v1/conversation/closing-message`입니다. 나머지 생성 API들은 앞으로 이 서버가 맡을 책임 방향이며, 아직 구현된 엔드포인트가 아닙니다.
...
app/
  ...
  conversation/
    application/
      next_message_service.py  다음 AI 메시지와 종료 메시지 생성 use case
  models/
