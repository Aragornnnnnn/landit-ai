...
def _watched_patterns_section() -> str:
    return (
        ...
        "TENSE, SUBJECT_VERB_AGREEMENT, and NEGATION a verb; for VERB_FORM only the second "
        "verb in a chain, the one that follows another verb, a modal, be, or to (decided to "
        "leave, keeps asking, can swim), with span covering the whole chain (can swim, not "
        "swim), never a main verb standing on its own such as has or lives; for ARTICLE the word "
        "a, an, or the with its noun, or a singular countable noun that is missing one; for "
        "PLURAL a noun whose number matters; for PRONOUN a third-person pronoun such as he, "
        ...
        "going) is VERB_FORM, not TENSE, and a lone verb in the wrong time (I buy it last "
        "week) is TENSE, not VERB_FORM. A negative formed without do or be (We not agree) is "
        "NEGATION, not VERB_FORM. A missing plural ending (two cousin) is PLURAL, not "
