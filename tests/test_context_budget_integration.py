...
class ContextBudgetIntegrationTests(unittest.TestCase):
    ...
    def test_follow_up_and_continue_repairs_fit_before_any_provider_call(self):
        for mode in ("NORMAL", "CONTINUE_AFTER_EXIT_DECLINED"):
            with self.subTest(mode=mode):
                ...
                blocked = FakeOpenAI(contents=[json.dumps(reply) for reply in replies])
                response = self._post(
                    "/api/v1/free-talk/turn", payload, blocked, budget=max(sizes) - 5,
                )
                self.assertEqual(response.status_code, 400)
                self.assertEqual(response.json()["error"]["code"], "FREE_TALK_CONTEXT_TOO_LARGE")
                self.assertEqual(blocked.completions.calls, [])

    def test_oversized_inner_thought_starts_neither_generation_nor_correction(self):
        fake = FakeOpenAI(contents=[json.dumps(inner_thought_completion())])
        with patch("app.free_talk.application.conversation_service.ThreadPoolExecutor") as pool:
            response = self._post(
                "/api/v1/free-talk/inner-thought",
                payload_with_partner_turn(contextPolicyVersion="v1"), fake, budget=1,
            )
        ...
        self.assertEqual(fake.completions.correction_calls, [])
