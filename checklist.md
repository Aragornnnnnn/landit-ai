...
- [ ] 판정 내부 모델과 사용자 발화 근거 검증의 RED 테스트 작성.
- [ ] 판정 전용 첫 호출과 서버 `feedbackType` 잠금 구현.
- [ ] 근거 기반 문구 생성과 필수 플레이스홀더 검증 구현.
- [ ] 문구 복구 1회와 검증 실패 처리 구현.
- [ ] 3개 이상 세션 점수 가중치와 단일 별점 매핑 구현.
- [ ] 품질 평가 fixture와 도구를 판정·문구 분리 구조로 수정.
- [ ] 전체 unittest, compileall, pip check, OpenAPI, diff check 실행.
- [ ] 중요 사례 14개 3회와 전체 115개 실제 모델 재평가.
