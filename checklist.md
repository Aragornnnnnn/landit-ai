...
- [ ] 판정 형식 검증 실패의 1회 복구 RED 테스트 작성.
- [ ] 판정 복구 재실패 시 502와 cache 미저장 테스트 작성.
- [ ] provider 실패를 복구하지 않고 503을 유지하는 테스트 보강.
- [ ] 판정 복구 prompt와 최소 구현 추가.
- [ ] 내부 cache와 품질 결과에 `judgementWasRepaired` 기록.
- [ ] 고정 품질 사례 7개를 각 3회 실제 모델 평가.
- [ ] 중요 사례 14개를 각 3회 실제 모델 평가.
- [ ] 전체 115개 실제 모델 평가에서 형식 실패 최대 1건 확인.
- [ ] 외부 API와 OpenAPI 계약 유지 확인.
- [ ] 전체 unittest, compileall, pip check, diff check 통과.
- [ ] 구현과 품질 결과를 논리 단위로 커밋.
