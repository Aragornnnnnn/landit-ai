...
- 이전 split judgement/copy 구조는 LLM 판정 뒤 서버가 영문 단어 겹침, 질문별 템플릿, 플레이스홀더 규칙으로 의미를 다시 판정했다. 실제 발화에서 정상 답변을 거부하고 테스트가 구현 세부사항에 과도하게 묶이는 근본 원인이었다.
- `MessageFeedbackEvaluation`이 전체 내부 피드백과 `scoreEvidence`를 함께 가진다. 첫 호출은 전체 후보를 만들고, 두 번째 호출은 원본 요청과 후보를 기준으로 최종 전체 피드백을 다시 작성한다. 서버는 Pydantic 필드 계약, `GOOD`과 점수의 관계, placeholder 형식, catalog 근거만 검사한다.
- 구조 오류는 후보·검수 단계별로 한 번만 복구한다. 복구 프롬프트에는 Pydantic의 첫 구조 오류만 전달해 모델이 필드 계약을 수정할 수 있게 했고, provider·네트워크 실패는 기존 503으로 그대로 전파한다.
...
## 2026-07-17 LAN-167 판정 잠금과 스피킹 표기 차이 배제

- 같은 7개 사례에서 `origin/main`은 형식 실패 0건이었지만 피드백 유형은 5건, 점수와 문맥 경계까지 포함하면 4건만 기대값과 일치했다. 현재 최종 검수 결과는 7건 모두 일치했지만 1차 후보는 구조 복구가 반복되고 복구 후에도 실패하는 사례가 확인됐다.
- 1차 호출은 점수와 fallback 가능한 전체 후보를 만들고 서버가 `scoreEvidence`에서 `feedbackType`을 생성한다. 2차 호출은 잠긴 점수와 유형에 맞는 사용자 문구만 만들며, 실패하면 검증된 1차 후보를 사용한다.
- 백엔드는 모든 사용자 메시지와 같은 개수·순서의 피드백을 요구하므로 실패 메시지를 제외하지 않는다. 1차 유효 후보가 있는 메시지는 2차 실패와 관계없이 캐시에 저장한다.
- 사용자 발화와 교정 표현이 대소문자, 문장부호와 공백을 제외하면 같을 때는 스피킹 관점에서 동일한 발화로 본다. 대문자, 소문자, 쉼표, 마침표와 문장부호를 개선 이유로 설명하는 사용자용 문구도 거부한다.
- 첫 번째 provider 호출 자체가 실패한 경우에는 정확한 점수를 만들 수 없으므로 임의의 점수나 일반 피드백을 저장하지 않고 기존 오류 계약을 유지한다.

## 2026-07-17 LAN-167 판정 잠금 구현 계획 승인

- 1차 출력에서 `messageId`와 `feedbackType`을 제거하고, 서버가 요청 식별자와 `scoreEvidence`로 최종 유형을 조립한다.
- 1차 후보는 유형별 필드 정리, 플레이스홀더 형식, 스피킹 표기 차이 검증까지 통과한 뒤에만 fallback 후보가 된다.
...
- 실제 저장 데이터의 최신 9건은 `baseLocaleAnalogy`에 한국어 비유 대신 질문 충족 여부와 개선 행동을 직접 설명했다.
- `baseLocaleAnalogy`는 인용된 한국어 발화와 `것과 같아요` 또는 `것과 비슷해요` 비교를 포함해야 한다. `한국어로 치면`, `한국어로는`, `한국어로도` 접두사는 저장 전에 제거한다.
- 생성 후보와 최종 문구 검수에 같은 규칙을 적용하고, 설명형 문구는 기존 단계별 복구 경로로 다시 생성한다.
- 점수, `feedbackType`, 다른 사용자용 피드백 필드와 외부 API 계약은 변경하지 않는다.
