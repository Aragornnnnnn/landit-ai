...
### Task 13: 언어 정확도 점수에 근거와 승인 교정을 요구한다
...
`languageAccuracy<2`인데 교정 목록이 없거나, 2인데 목록이 있으면 거부한다. evidence가 사용자 발화에 없으면 `message_feedback_judgement_language_correction_evidence`로 거부한다.

- [x] **Step 2: 판정 모델과 근거 검증을 구현한다.**

내부 `MessageFeedbackJudgement.languageCorrections`에 evidence와 replacement를 추가하고 점수 조합과 사용자 발화 포함 여부를 검증한다.

- [x] **Step 3: 판정 프롬프트와 내부 출력 스키마를 갱신한다.**

0 또는 1이면 가장 작은 사용자 발화 evidence와 그 구간만 고친 replacement를 한 개 이상 요구하고, 2면 빈 배열을 요구한다.
...
- [x] **Step 8: 언어 교정 replacement를 허용 어휘의 근거로 사용한다.**
