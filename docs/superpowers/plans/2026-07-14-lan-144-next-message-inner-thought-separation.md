...
- [x] 성공 테스트의 예상 응답에서 `innerThought`, `innerThoughtType`을 제거한다.
- [x] system prompt에 `innerThought` 출력 지시가 없고, 이전 5개 필드 모델 응답은 `AI_RESPONSE_INVALID` 502가 되는 테스트를 추가한다.
- [x] `.venv/bin/python -m unittest tests.test_conversation_api.NextMessageApiTests`를 실행해 RED를 확인한다.
- [x] `NextMessageResponse`에서 속마음 필드를 제거하고 validator 대상을 `aiMessage`, `translatedMessage`로 줄인다.
- [x] `_next_message_system_prompt()`에서 속마음 정책과 속마음 출력 필드를 제거한다. 고정 질문, 맞장구, 목표 달성 상태 정책은 그대로 둔다.
- [x] 같은 focused test를 다시 실행해 성공, 고정 질문 누락 502, 생성 실패 503, 요청 불일치 400이 모두 통과하는지 확인한다.
- [x] 아래 논리 단위로 커밋한다.
...
- [x] `valid_next_message_payload()`에서 `nextQuestion`만 제거한 `valid_inner_thought_payload()`를 추가한다.
- [x] `InnerThoughtApiTests`에 다음 테스트를 먼저 작성한다.
...
- [x] `.venv/bin/python -m unittest tests.test_conversation_api.InnerThoughtApiTests`를 실행해 RED를 확인한다.
- [x] `InnerThoughtRequest`에 양수 ID, 최소 1개 히스토리, 마지막 USER 메시지 일치 검증을 추가한다.
- [x] `InnerThoughtData`는 `extra="forbid"`로 OpenRouter 출력이 `innerThought`, `innerThoughtType`만 포함하도록 검증한다.
- [x] `generate_inner_thought()`에서 별도 OpenRouter 호출 후 `InnerThoughtData`를 검증하고 요청 식별자로 `InnerThoughtResponse`를 조립한다.
- [x] 기존 `next-message`의 `Inner Thought Policy`와 속마음 예시를 전용 system prompt로 옮긴다. 다음 질문·행동 계획·문법 평가 금지 정책을 유지한다.
- [x] user prompt에 시나리오, 전체 히스토리, 마지막 USER 메시지를 넣고 `nextQuestion`은 넣지 않는다.
- [x] `app/api/conversation.py`에 route를 추가하고 기존 502·503 오류 매핑을 그대로 적용한다.
- [x] focused test를 다시 실행해 성공, 400, 502, 503 경로를 확인한다.
- [x] 아래 논리 단위로 커밋한다.
...
- [x] `next-message` 응답 필드 목록에서 속마음 필드를 제거한다.
- [x] `inner-thought` 요청 검증, 응답 식별자 복사, 재호출 의미, BE 상태 관리 책임, 종료 턴 제외를 문서화한다.
- [x] `checklist.md`와 `context-notes.md`에는 LAN-144 구현 결과와 검증 결과만 이어서 기록한다.
- [x] 아래 검증을 순서대로 실행한다.
