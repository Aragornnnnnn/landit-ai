...
- 정책이 없는 AI 요청은 기존 전체 이력 경로를 유지한다. `v1` 요청만 system·user·응답 schema의 JSON과 512토큰 여유분을 계산한다. 초과하면 오래된 완료 구간부터 제외하고 요약 전체를 버리며 `historyIncomplete=true`로 표시한다. 직전 AI·현재 USER 원문도 들어가지 않으면 400을 반환한다. 일반 대화·속마음·종료 및 repair 계약을 검사하며 원본 요청 객체는 변경하지 않는다.
