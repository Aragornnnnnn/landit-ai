...
AI가 예산을 초과하면 오래된 완료 원문 구간부터 제외하고 다시 계산한다. 제외한 구간이 요약되지 않은 범위라면 해당 요청은 `historyIncomplete=true`이며 기존 요약 전체를 사용하지 않는다. 빠진 구간에 정정이 있을 수 있기 때문이다. 직전 AI·현재 USER만으로도 예산을 넘으면 LLM을 호출하지 않고 입력 길이 오류를 반환한다.

일반 대화 내부 오류 코드는 `FREE_TALK_CONTEXT_TOO_LARGE`(HTTP 400)를 AI·BE에 추가한다. BE는 해당 코드만 명시적으로 매핑하고 예약 보상 후 동일한 400을 반환한다. 기존 프리톡 처리 실패 시의 원문/재전송 정책을 유지하며 임의 삭제를 추가하지 않는다. FE는 기존 오류 봉투로 재입력을 안내할 수 있는지 통합 검증한다. 시나리오 입력이나 기존 일반 AI timeout에는 이 정책을 적용하지 않는다.
...
| USER와 직전 AI만으로 예산 초과 | LLM 호출 없이 명시적 400, 기존 예약 보상. 현재 발화를 잘라 성공한 것처럼 처리하지 않음. |
...
2026-09-20 기준 `feat/LAN-531`에 AI·BE 최소 구현을 반영했다. AI는 선택적 컨텍스트 필드와 요약 endpoint, 요약 참조 검증, 입력 예산 선검사, 8초 deadline·SDK 재시도 0회를 제공한다. BE는 feature flag와 사용자 allowlist를 기본 비활성으로 두고, 요약 테이블·선점·best-effort 비동기 실행·최근 요약 경계 이후 원문만 전달하는 연결부를 추가했다. 요약 전용 BE HTTP timeout은 10초이며 일반 AI `requestTimeout`과 분리했다.

토큰 수는 배포 모델 tokenizer가 아닌 UTF-8 바이트 기반 추정치로 시작했으므로 실제 `usage.prompt_tokens`와 대조해 보정해야 한다. 현재 BE 요약 작업은 기존 `applicationTaskExecutor`를 재사용하므로 설계에 적은 전용 2개·대기 8개 실행기 상한은 운영 활성화 전에 별도 검증·분리해야 한다. 소프트 삭제 연결도 같은 시점에 확인한다. 기능 flag가 꺼져 있는 동안에는 기존 전체 이력 경로를 유지한다.
