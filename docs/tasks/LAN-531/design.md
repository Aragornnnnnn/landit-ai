...
작성일: 2026-09-20. 이 문서는 AI·BE 구현 인수인계의 단일 기준이다. 이전 임시 실행 계획보다 이 문서의 계약과 실패 처리를 우선한다. 이번 변경은 설계 문서만 포함하며, 구현은 사용자가 선택한 하위 모델이 별도로 진행한다. 수치들은 검증을 시작할 기본값과 목표이며 운영에서 검증된 성과가 아니다.
