...
아니라고 명시된 인용·가정·역할극·학습 예문은 기억 후보로 만들지 않는다. 질문에
전제된 내용만으로 사실을 추론하지 않되, 사용자가 사실을 명시하며 기억해 달라고
요청한 경우는 허용한다.

AI 응답 검증 후에는 명백한 오기억만 결정적으로 제거한다. 모호한 상대 요일을 근거로
만든 `EVENT`, 단순 재요청을 저장한 후보, 명시적으로 부정한 학습 예문, 상대
시간이 그대로 남은 후보와 명시적 사실이 없는 순수 질문에서 만든 후보가 대상이다.
제거 후 남은 후보는 0부터 다시 인덱싱하며, 누락된 후보를 규칙으로 새로 만들지는
않는다.
...
`supersededMemoryIds`를 가질 수 있다. 후보가 기존 기억의 참여자·장소·주기 같은
세부 정보를 제거한 일반화라면 기존 기억을 대체하지 않고 `IGNORE`로 보정한다.
...
- `usedMemoryIds`는 요청의 `memoryContext`에 포함된 ID만 허용한다.
- 번역 응답에 기억의 구체 단어가 드러난 경우에만 사용으로 기록한다.
- 여러 구체 단어를 가진 기억은 한 단어의 일반적인 겹침만으로 사용 처리하지 않는다.
