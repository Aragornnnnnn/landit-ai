...
| 발음 | compare, accent check, error description | 오디오 멀티모달, 프롬프트 JSON, 전용 parser와 제한 재시도 | 적용. 판정 유형별 Pydantic 모델을 사용하고 기존 parser와 17초 전체 예산을 유지한다. |
...
- Provider가 `response_format`, `json_schema`, Structured Outputs 또는 호환 endpoint 부재를 400·404·422로 명시하면 `json_schema`에서 `json_object`, 다시 기존 프롬프트 JSON 순서로 fallback한다. 발음 호출은 남은 17초 전체 예산 안에서만 다음 형식을 시도하며, 판정 재시도는 이미 선택된 형식을 재사용한다.
...
| `google/gemini-3.5-flash` / Google AI Studio 고정 | 실제 발음 compare schema, 무음 오디오 2개 | 성공, strict Pydantic 검증 통과 | 2,636ms |
| `google/gemini-3.5-flash` / Google AI Studio 고정 | 실제 accent schema, 무음 오디오 1개 | 성공, strict Pydantic 검증 통과 | 2,594ms |
| `google/gemini-3.5-flash` / Google AI Studio 고정 | 실제 error description schema, 무음 오디오 2개 | 성공, strict Pydantic 검증 통과 | 3,142ms |
...
- strict schema 생성, malformed JSON 1회 재시도, `json_schema → json_object → prompt` Provider 미지원 fallback, timeout 비-fallback을 단위 테스트한다.
