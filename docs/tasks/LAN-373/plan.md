...
- [x] 전량 S3 게시 완료 (mp3 29,157 + 매니페스트·reference, 충돌 0) — 개수 장부와
  게이트 실측은 [gates.md](./gates.md)
- [x] 출시 전 게이트 2건 통과 — 지연 기준 재정의(무오류 p50≤5초·오류 p90≤12초) 및
  EN_AU 억양 대조 전면 비활성화 정책 확정 (기획 추인). FE 로딩 UI 후속 이슈 필요
- [x] EN_GB/AU 발음 사전 — espeak-ng 도입으로 해소, 981개 전량 기준 데이터 생성 완료
- [x] EN_AU 출시 범위 — 게이트 A 실측(호주 발화 오탐)으로 억양 대조 전면 비활성화 확정,
  본 판정(발음·강세)만 제공 ([gates.md](./gates.md))
