...
- **`followUpAsked`는 모델의 자기 보고를 그대로 믿지 않는다.** 실측에서 모델은 자기 질문("어디로 등산 가?")을 하고도
  ...
  어간 포함으로 본다. 질문과 번역문이 같은 기준 언어라 가능한 비교다. 잘못 `false`가 되면 다음 세션에 한 번 더 물을 뿐이다.
- **첫 턴은 질문이 빠지면 한 번 복구 재호출한다**(`free_talk_turn_follow_up_repair`). 기존 턴 프롬프트의 "반응한 뒤 후속
  ...
  않았거나 종료 판정을 뒤집으면 첫 응답을 그대로 쓴다. 세션당 첫 턴 한 번, 질문이 빠졌을 때만 추가 호출이 생긴다.
  오프닝은 실측 39/40이라 복구를 두지 않았다.
...
| 첫 두 턴 안에 질문 (오프닝) | 10세션 | 10/10 | **10/10** |
| 첫 두 턴 안에 질문 (USER_FIRST 첫 턴) | 10세션, 메시지를 직접 읽어 판정 | 8/10, 거짓 `true` 2건 | **9/10**, 거짓 `true` 0건 |
