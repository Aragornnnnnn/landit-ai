...
| 기억 근거 교정 | `inner-thought` | `memoryContext` (최대 3) | `correction.usedMemoryId`, `correction.memoryLabel` |
...
- **"질문했나"와 "내보내도 되나"는 따로 본다.** 후속 질문이 학습 언어가 아니라 기준 언어로 메시지에 들어가면(질문 원문을
  붙여 넣었거나, 기준 언어 절로 풀어 썼거나) 누출이다. 절은 한글 단어가 셋 이상 이어진 경우만 보며, 사용자가 방금 쓴 말을
  받아 준 것은 제외한다. "떡볶이 sounds amazing" 같은 단어 하나는 정상 대화다. 누출이 있으면
  `workflow=free_talk_follow_up_base_locale_leak` 경고를 남기고 한 번 복구한다. 채택 순서는 **묻는 데 성공한 복구 응답 >
  깨끗한 첫 응답 > 깨끗한 복구 응답**이고, 둘 다 샜을 때만 `AI_RESPONSE_INVALID`(502)다. 복구 호출은
  `free_talk_follow_up_repair_timeout_seconds`(기본 8초, 재시도 없음) 안에서만 돈다.
- (이전 구현 기록) **기준 언어 질문 원문을 `aiMessage`에 붙여 넣은 응답은 묻지 않은 것으로 본다.** 실측에서 모델이 한국어 질문을 영어
...
백엔드는 지난 스몰톡 상세 화면의 교정 카드에 "9/13 스몰톡에서 말한 헬스장" 같은 태그를 그린다. AI가 돌려주는 것은 기억 ID뿐이고
저장된 `content`는 "사용자는 집 앞 헬스장에 다닌다" 같은 문장형이라, 짧은 명사구를 뽑으려면 뜻을 이해해야 한다. 그래서 교정 LLM이
**같은 호출 안에서** 라벨을 함께 돌려준다. 추가 호출은 없다.

- **역할 분담.** AI는 근거 기억이 가리키는 대상을 기준 언어의 짧은 명사구로 돌려준다(`"헬스장"`). 날짜와 "스몰톡에서 말한"이라는
  틀은 백엔드가 `observedAt`으로 붙인다. 모델이 날짜를 지어낼 여지를 주지 않기 위함이다(LAN-454 핫픽스와 같은 취지).
- **라벨이 나빠도 교정은 죽이지 않는다.** 서버 검증(`memory_label_rejection`)에 걸리면 `memoryLabel`만 `null`로 내리고 교정 문장과
  `usedMemoryId`는 유지한다. 백엔드는 라벨이 `null`이면 "{날짜} 스몰톡에서 말한 내용"이라는 기본 문구로 태그를 그리기 때문이다.
  거절 사유는 `without_memory_id | blank | too_long(20자 초과) | invalid_chars(줄바꿈·!?.) | contains_date(숫자로 된 날짜 표기)`이며
  `workflow=free_talk_correction_memory_label_dropped reason= sessionId= messageId=` 경고를 남긴다. 라벨 원문에는 기억 내용이 담기므로
  로그에 쓰지 않는다. `다음 주 면접`처럼 숫자 없는 표현은 날짜 표기로 보지 않는다.
- 요청에 없던 기억 ID라 `usedMemoryId`가 버려지면 라벨도 함께 버린다(그쪽 경고가 이미 남으므로 라벨 경고는 따로 남기지 않는다).
  근거 없는 관사 교체라 교정이 버려지면 라벨도 없다.
- **기억이 없는 요청은 프롬프트와 스키마가 라벨 도입 전과 글자까지 같다.** 라벨 절(`Memory Label:`)과 라벨 필드가 있는 structured
  output 하위 모델은 `memoryContext`가 있을 때만 쓴다. 오프닝의 `pendingFollowUp`과 같은 패턴이다.
- **라벨 규칙은 `Memory Grounding:` 절에 넣지 않고 출력 스키마 직전의 별도 절로 둔다.** 처음에는 요청대로 Memory Grounding 절 안에
  붙였는데, 같은 항목·같은 횟수의 A/B에서 기억 근거 교정이 54% → 44%로 줄었다. 기억을 근거로 쓸지 판단하는 자리에 "쓰면 라벨도
  지어야 한다"는 부담과 금지 규칙이 같이 놓인 탓으로 본다. "판단을 먼저 하고 그 뒤에 표기만 한다"고 명시한 별도 절로 옮기자
  53% 대 52%로 차이가 사라졌다.
- 프롬프트의 좋은 예·나쁜 예는 holdout에 없는 대상(빵집, 고양이, 회사 동기, 치과 예약)으로 썼다. 요청서의 예시(헬스장, 수영장,
  강아지)는 holdout 기억과 겹쳐 그대로 넣으면 측정이 오염된다.
...
- **신규 필드 `correction.memoryLabel`**(nullable string)은 명세서에 없다. Notion의 AI-BE 속마음 명세에 추가해야 한다.
...
백엔드 역직렬화가 모르는 필드를 허용하는지 배포 전에 확인한다. `memoryLabel`은 백엔드가 `memoryContext`를 보내기 전까지 항상
`null`로 나가며, 백엔드는 모르는 필드를 무시하므로 영향이 없다.
...
- `tests/test_free_talk_correction_rules.py`: 관사 교체 판별 규칙, 라벨 거절 규칙(날짜 표기 3종 거부와 숫자 없는 표현 통과, 길이 경계).
- 라벨: 기억 근거 교정이면 ID와 함께 옴, 기억을 안 썼으면 둘 다 null, 거절 사유 5종마다 라벨만 null + 경고 1건, 모르는 기억 ID·버려진
  교정과 함께 사라짐, 로그에 라벨 원문 없음, 기억 없는 요청은 프롬프트·스키마에 라벨이 없음, OpenAPI nullable string.
...
### 화면용 라벨 실측 (2026-09-19, 같은 모델)

holdout의 기억 근거 교정 12종에 고유명사 항목 4종(식당 이름, 사람, 반려동물 이름, 일정)을 더해 기억 있음 16종 × 5회, 기억 없음·무관한
기억 각 36회, 회귀 세트 48회를 돌렸다. 프롬프트 예시로 쓴 대상은 하나도 포함하지 않았다.

| 항목 | 결과 |
| --- | --- |
| 기억 근거 교정 중 라벨이 서버 검증을 통과 (목표 ≥ 95%) | **41/41 (100%)**, 앞선 회차 31/31. 거절 0건 |
| 통과한 라벨이 "스몰톡에서 말한 ___"에 자연스러움 (목표 ≥ 90%) | 직접 읽어 **41/41**. 예: `헬스장`, `단골 카페`, `동네 수영장`, `회사 근처 도서관`, `새로 이사한 아파트`, `독서 모임`, `팀장님 면담`, `햄스터 콩이` |
| 고유명사 표기 | `온더테이블`, `햄스터 콩이`로 기억 표기 그대로 |
| 날짜·문장형·학습 언어가 섞인 라벨 | 0건 |
| 기억을 근거로 썼는데 라벨이 빠짐 / 기억 없이 라벨만 옴 | 0건 / 0건 |
| 기억 ID 정확도 | 41/41 |
| 회귀: 기억 없는 세트 | 교정 0/36, 멀쩡한 발화 오교정 0/24, 실수 포착 + 기대 패턴 24/24, 지연 p50 1.10 / p90 1.42초 (프롬프트 불변) |
| 회귀: 기억 근거 교정 비율 (A/B, 12종 × 8회씩) | 라벨 없음 50·54·50·52% / 라벨 있음(최종 구조) 58·53% |
| 지연: 기억 있는 요청 (A/B) | 교정이 나올 때 p50 1.38 → 1.55초, 안 나올 때 1.04 → 1.11초. 전체 p90 1.8 → 2.1초 |

- 같은 기억에 라벨이 회차마다 조금 다르게 나온다(`도서관` / `회사 근처 도서관`, `아파트` / `새로 이사한 아파트`). 둘 다 자연스러워 그대로 둔다.
- **지연은 기억이 있는 요청에서만 0.1~0.2초 늘었다.** 기억이 없는 요청은 그대로다. 교정 호출은 속마음 호출과 병렬로 돌고 6초에서 끊기므로
  엔드포인트 지연은 max(속마음, 교정)이다.
- 무관한 기억 조건에서 1/36건이 기억을 인용했다: 요가 학원 발화에 헬스장 기억이 짝지어지자 `at the studio`로 고치며 라벨을 `헬스장`으로
  달았다. 항목을 16종으로 늘리면서 짝이 바뀌어 생긴 실제로 애매한 조합이지만, 뜻이 가까운 다른 기억을 끌어다 쓸 수 있다는 위험은 맞다.
  라벨 도입 전에는 이 짝을 재지 않았으므로 라벨 때문인지는 알 수 없다. 운영에서 본다.
- 20자 상한에 걸린 라벨은 없었다. 가장 긴 라벨은 `새로 이사한 아파트`(10자)다.

PR 직전 두 번째 독립 리뷰가 위 한글 누출 규칙의 회귀를 재현했다: 한글이 한 글자만 있어도 누출로 봐서, 질문을 영어로
제대로 하고 "떡볶이 sounds amazing"이라고 받아 준 정상 응답이 502가 됐다. 규칙을 절 단위로 좁히고 채택 순서를 위 4절처럼
바꾼 뒤 같은 holdout을 두 번(각 140건) 다시 돌렸다: 한국어가 나간 메시지 0건, 502는 280건 중 1건, 오프닝 119/119,
첫 턴 120/120(서버 판정), 첫 턴 p90 1.8초. 누출 자체는 회차마다 1~7건 생기며 복구 재호출이 거의 다 살린다.
`askedMemoryIds` 중복을 400으로 거절하는 것은 보조 입력이 본 응답을 막는다는 지적이 있었으나 명세서 5절의 계약이라 유지했다.
