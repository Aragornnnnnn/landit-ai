...
## 운영 근거 검증 hotfix — 2026-09-21

### 확정 범위.

- `origin/main`의 `0376cf0`에서 `hotfix/LAN-438`을 생성했다. 운영 AI의 실행 이미지 digest와 같은 커밋임을 확인했다. 기존 `feat/LAN-438`과 다른 작업의 변경은 포함하지 않는다.
- 한국어만 있는 답변·인용문을 영어 능력의 관찰 근거로 인정하는 오류를 수정한다. 서버는 비라틴 문자만 있는 근거를 `NOT_OBSERVED / level=null / evidenceExcerpt=null`로 제외한다. 번역을 상상해 평가하지 말라는 프롬프트 후보도 검증했지만, 다른 영역의 점수 흔들림을 배제할 수 없어 최종안에서는 기존 프롬프트를 그대로 유지한다.
- 먼저 기존 Pydantic·메시지 ID·원문 인용 검증을 수행한다. 유효하지 않은 Core를 언어 필터로 정상 처리하지 않는다. 유효한 Core 안에서 제외한 영역만 교체하고 다른 영어 관찰값·메시지 순서는 보존한다.
- 점수 근거를 제외했다면 이전 점수를 설명할 수 있는 Details는 함께 버린다. 언어 근거 부족 자체로 추가 LLM 재시도를 만들지 않는다. 기존 Core 복구 1회·출력 형식 fallback·호출 deadline은 유지한다.
- 한국어 이름이 들어간 영어 문장, `Saturday.`, 숫자·시간, 악센트·전각 라틴 문자를 일괄 차단하지 않는다. 이 필터는 언어 판별 모델이 아니다. 스페인어 등 라틴 문자 언어 및 혼합 문장 내 실제 영어 능력은 여전히 LLM 판단 대상이다.
- 질문·프롬프트·가중치·관찰 상한·초기화·승급 정책·API 스키마·DB 구조는 변경하지 않는다. 과거 평가·프로필을 소급 수정하지 않는다.

### 추가 검토 후 제외한 변경.

| 검토 항목 | 판단 |
| --- | --- |
| 짧은 선택 답변의 문법을 비관찰로 전환 | 후보 실측에서 `NOT_OBSERVED + level=1` 모순과 재시도 실패가 발생했다. 이번 hotfix에서 제외하고 기존 기준을 유지한다. |
| 영어 근거 한정 프롬프트 추가 | 한국어 표본은 개선됐지만 짧은 선택 답변의 discourse 1 판정도 관찰됐다. 인과관계는 입증되지 않았으나 최소 hotfix에서는 제외한다. 비라틴 근거는 서버에서 차단한다. |
| 최근 경험 질문에 직업·취미로 답한 경우 | 합성 비교에서 기존·후보 모두 `PARTIAL`로 판단했다. 운영 오류가 해결됐다고 볼 수 없으므로 추가 보정은 보류한다. |
| Level 2~3 쏠림 | 고수준 합성 표본은 기존·후보 모두 Level 4였다. 운영 분포만으로 상한·가중치를 변경하지 않으며, 사람 기준표 검증이 필요하다. |
| confidence 1.00 | 정확도 확률이 아니라 BE의 가중 관찰 비율이다. 정의·공식은 유지한다. |

### BE 호환성과 검증 한계.

- 운영 BE `feed8862`의 `TextLevelAssessmentPolicy`, `LearningLevelPolicy`, 요구도·질문 그룹 enum을 그대로 추출해 Java 21로 실행했다. 전체 비관찰, HIGH 답변 1개 비관찰, 문법 관찰 부족의 3개 사례에서 최초 초기화·승급 모두 차단되고 현재 수준 4와 streak 1이 유지됐다. 유효한 관찰의 최초 초기화도 유지됐다.
- 부분 관찰은 `MODEL`이며 모든 근거가 없어도 유효한 Core라면 `MODEL`로 저장될 수 있다. 측정 수준이 없을 때의 `displayLevel`은 기존 수준 또는 3이며, 새 영어 측정값으로 간주하면 안 된다. 프로필 반영은 `sufficientEvidence`가 결정한다.
- BE 코드를 수정하지 않았으며 전체 Gradle·실제 DB 저장·FE 화면·결제 E2E 검증을 실행한 것은 아니다.
- BE `assessmentVersion=text-level-v1.3`은 계산·적용 정책 버전으로 유지한다. 이 값만으로 AI 검증 코드 변경 전후를 구분할 수 없으며, 이번 비교는 AI 커밋·프롬프트 해시와 배포 시점을 함께 사용해야 한다.

### 검증 기록.

- 회귀 테스트: 한국어 근거 배제 4개 실패를 먼저 재현한 뒤 통과시켰다. 최초/재시도·부분 보존·전체 비관찰·한국어 이름·숫자·시간·다른 비라틴 문자·원문 불일치 거부를 검사한다.
- 최종 unittest: 544개 실행, 실패 0, 기존 환경 의존 skip 7개. `compileall`, `pip check`, `git diff --check` 통과. `git diff origin/main`으로 프롬프트가 원본과 같은 것도 확인했다.
- OpenAPI 변경 없음. 수정 전후 SHA-256: `b9a1e566d515e0f40badba2690aed9de941fd1aedd56abf0e4ab03b8da6f3be8`.
- 실제 모델 비교는 사용자 승인 총 1달러 이내에서 합성 입력만 사용한다. 제품 모델은 SSM에서 확인한 `openai/gpt-5.4-mini`이며, 작업 실행 모델이나 별도 기준 평가 모델과 혼동하지 않는다. 사람 라벨 기반 블라인드 정확도 검증은 아니다.
- 입력 7종 × 3회씩 기존(`baseline`), 짧은 답변 변경 포함 후보(`candidate`), 영어 근거 프롬프트 후보(`final`), 최종 서버 검증 단독안(`guard_only`)을 비교한다. `final`은 중간 실험 파일명이며 채택된 코드가 아니다. 매 호출 전 공식 모델 가격과 최대 토큰·입력 크기로 비용 상한을 검사한다. 원문·실행기·프롬프트 해시·사용량은 로컬 `/private/tmp/lan438-hotfix-eval-20260921.S7cx3x/`에 보존하며, 운영 사용자 데이터·API 키는 저장하지 않는다.

| 실험 | 평가 횟수 | 유효 Core | 재시도 포함 호출 | 실측 비용 |
| --- | ---: | ---: | ---: | ---: |
| 기존 main | 21 | 19 | 28 | $0.12317670 |
| 짧은 답변 변경 포함 후보 — 보류 | 21 | 20 | 26 | $0.11305230 |
| 영어 근거 프롬프트 후보 — 보류 | 21 | 21 | 24 | $0.10642200 |
| 서버 검증 단독안 — 채택 | 21 | 21 | 24 | $0.09934350 |
| 합계 | 84 | 81 | 102 | **$0.44199450** |

- 최종안: 한국어 전용 표본 3/3은 전 영역 비관찰·측정 수준 없음, 영어 3답변+한국어 1답변 표본 3/3은 영어 근거 보존·불충분 판정, 한국어 이름 포함 표본 3/3은 정상 Level 3이었다. 기존 한국어 전용 표본은 2/3에서 Level 3·충분한 근거로 잘못 인정됐다.
- 최종안의 첫 응답 이후 Core 재시도 3회는 모두 복구됐다. p50 4.963초·최대 20.654초이며 지연 개선을 입증하는 실험은 아니다. 호출별 provider 사용량의 cost 값이 모두 존재했다.
- 주제 이탈·짧은 답변·사람 평가 대비 정확도 문제를 해결했다고 해석하지 않는다. 최종안의 기존 루브릭 해시는 baseline과 동일하다. 비용은 이번 실행 합계이며 운영 세션당 비용 추정과 구분한다.
- 로컬 코드·테스트 커밋 완료 후에도 PR 생성·병합·운영 배포·DB 변경은 별도 단계다.

## 현재 정책 보완 — 2026-09-11

- 사용자 최종 확인: BE는 최초 유효 평가 1회만 기존 자가선택 수준을 대체하고 `INITIALIZED`를 반환한다. 이후에는 기존 2회 연속 상향 근거에 따른 한 단계 승급·자동 강등 없음 정책을 유지한다. 무효·근거 부족·오래된 결과는 최초 확정 기회를 소진하지 않는다. 상세 계약은 BE의 동일 이슈 plan.md가 기준이다.
- 공유 루브릭에 짧지만 정확한 선택/시점/연락 수단 답변의 대화 구성 기준과 과제 충족/능력 수준 분리를 보강한다. 첫 호출과 Core 재요청이 같은 기준을 사용하며, JSON 출력 협상·원문 검증·Details 격리는 변경하지 않는다.
- 필수로 묻지 않은 이유나 뒤의 시나리오 목표 때문에 PARTIAL을 주지 않는다. 반대로 이유를 명시적으로 물었다면 짧은 선택만으로 ACHIEVED를 주지 않는다.
- 단순 무오류 답변을 자동 L4/L5로 올리지 않는다. 짧은 문맥 연결은 L3의 근거가 될 수 있지만 실제 낮은 문장 통제·부정확한 표현·주제 이탈은 독립적으로 평가한다.
- AI 루브릭과 실행 manifest는 `text-level-v1.2`를 유지한다. 최초 1회 적용 여부를 구분하는 BE 정책 버전은 `text-level-v1.3`이며 AI 프롬프트·가중치·상한 변경은 없다. 과거 baseline과 결과 파일은 변경하지 않고, 새로운 경계 입력 3개는 사람 미검증 후보로 유지한다.
- BE PR #177의 매번 즉시 대체는 사용자 의도를 넓게 해석한 구현이었다. 최초 1회 대체와 이후 기존 승급 정책 유지로 바로잡는다. 이전 블라인드 결과는 당시 프롬프트의 증거이며 이번 정책 정정 검증과 구분한다.
- 검증: `.venv/bin/python -m unittest discover -s tests` 534개 실행, 실패 0·건너뜀 7. 최초/재시도 프롬프트 연결·경계 fixture·Core fallback과 독립 검수를 통과했다. 실제 모델 재판정은 실행하지 않았으며 단위 테스트만으로 판단 개선을 입증했다고 하지 않는다.
