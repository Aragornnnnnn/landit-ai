...
| 최근 경험 질문에 직업·취미로 답한 경우 | 앞선 기존·프롬프트 후보 비교에서는 모두 `PARTIAL`이었지만, 서버 검증 단독안의 반복 비교에서 1/3이 다시 `ACHIEVED`로 오판됐다. 프롬프트가 같은데도 흔들리는 잔여 문제이며 이번 hotfix로 해결되지 않는다. |
...
- 최종 결과의 의미 검증에서 주제 이탈 `ACHIEVED` 금지 assertion이 실패했다. 원문을 확인해 최종 3회 중 1회의 잔여 오판으로 분류했다. 전체 21/21 유효 Core는 형식·근거 계약의 성공이지 21/21 평가 정답을 뜻하지 않는다. 이 사례의 결정적 서버 판별은 질문별 문자열 예외로 만들지 않으며, 별도 평가 보정의 회귀 사례로 남긴다.
