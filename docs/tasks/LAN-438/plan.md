...
- 사용자 승인으로 BE는 유효하고 충분한 최신 평가 수준을 기존 설정에 즉시 대체한다. 상향/하향을 모두 허용하고, 무효·근거 부족·오래된 결과 보존과 멱등성은 유지한다. 상세 계약은 BE의 동일 이슈 plan.md가 기준이다.
...
- 새 실행 manifest/BE 정책 버전은 `text-level-v1.2`. 과거 baseline과 결과 파일은 변경하지 않고, 새로운 경계 입력 3개는 사람 미검증 후보로 추가한다.
- 기존 2026-09-05~07 계약의 2회 승급·자동 강등 없음은 최신 사용자 승인으로 대체됐다. 이전 블라인드 결과는 변경 전 프롬프트의 증거다.
