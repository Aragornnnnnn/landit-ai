...
- [ ] 후속 턴에서 `inferredTitle`이 생성되어도 502가 발생하지 않고 `null`로 응답한다.
- [ ] 종료 의사가 감지된 응답에 대화 메시지가 포함되어도 메시지를 제거하고 정상 응답한다.
- [ ] 첫 사용자 턴의 `inferredTitle` 형식 검증은 유지한다.
- [ ] 일반 대화 응답의 `userExitIntentDetected`, `aiMessage`, `translatedMessage` 필수 검증은 유지한다.
- [ ] 프리톡 대화 응답 필드별 검증 기준을 테스트로 명시한다.
