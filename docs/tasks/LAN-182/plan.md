...
- [ ] **Step 1: 추가 복구 없이 일관성 불일치 후보를 저장하는 실패 테스트를 작성한다.**
...
- [ ] **Step 2: 강한 검증 실패는 기존처럼 복구 후 `FAILED`인지 확인하는 테스트를 유지·보강한다.**
...
- [ ] **Step 5: 일관성 불일치는 복구하지 않고 경고 후 같은 후보를 파싱한다.**
...
- [ ] **Step 6: 대상 테스트와 전체 대화 API 테스트를 실행한다.**
...
- [ ] **Step 1: `primaryFeedbackDimension`이 없는 후보와 제한된 원문 매칭 테스트를 작성한다.**
...
- [ ] **Step 2: 대상 테스트를 실행해 RED를 확인한다.**

Run: `.venv/bin/python -m unittest tests.test_conversation_api`

Expected: 필수 대표 차원 또는 exact substring 검증 때문에 새 테스트가 실패한다.

- [ ] **Step 3: LLM 후보에서 대표 차원을 제거하고 실제 교정 근거를 서버가 확인한다.**
...
- [ ] **Step 1: 운영 품질 사례 fixture 계약 테스트를 작성한다.**
...
- [ ] **Step 2: 프롬프트 계약 테스트를 작성하고 RED를 확인한다.**
...
- [ ] **Step 3: 생성·검수 프롬프트에 최소 수정과 미완성 발화 기준을 추가한다.**
...
- [ ] **Step 4: 품질 평가 테스트를 실행한다.**

Run: `.venv/bin/python -m unittest tests.test_quality_evaluation tests.test_conversation_api`

Expected: 모든 테스트가 통과한다.

- [ ] **Step 5: 실제 OpenRouter 설정이 있으면 LAN-180과 LAN-182 fixture를 각각 3회 평가한다.**
...
- [ ] **Step 1: 세 문서에 같은 PR 메타데이터 규칙을 반영한다.**
...
- [ ] **Step 2: 전체 unittest를 실행한다.**

Run: `.venv/bin/python -m unittest discover -s tests`

Expected: 모든 테스트가 통과한다.

- [ ] **Step 3: OpenAPI와 diff를 검증한다.**
...
- [ ] **Step 4: 계획에 실제 검증 결과와 남은 위험을 기록한다.**
