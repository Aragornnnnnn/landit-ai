...
GitHub Actions의 `Verify application` 워크플로우는 `develop`, `main`을 대상으로 하는 PR과 두 브랜치로의 push에서 자동 실행됩니다. PR의 base 브랜치 변경도 검사하도록 `opened`, `synchronize`, `reopened`, `edited` 이벤트를 포함합니다.

워크플로우는 Python 3.12에서 의존성을 설치한 뒤 unittest, `compileall`, `pip check`, Docker image build를 순서대로 실행합니다. 실제 OpenRouter 호출과 secret 값은 CI에 포함하지 않습니다.
